...
4. **Try `Box::new(42)` vs `let x = 42`** — spot the allocator call in the heap version.

5. **Find a bounds check** — index a slice with a variable index in debug mode and find the panic branch in the assembly.
